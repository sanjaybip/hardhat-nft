import { expect } from "chai";
import { network, ethers, deployments } from "hardhat";
import { developmentChains } from "../../helper-hardhat-config";
import { DynamicSvgNft, MockV3Aggregator } from "../../typechain-types";

const lowSVGImageuri =
    "data:image/svg+xml;base64,";
const highSVGImageUri =
    "data:image/svg+xml;base64,";

const highTokenUri =
    "data:application/json;base64,";
const lowTokenUri =
    "data:application/json;base64,";

!developmentChains.includes(network.name)
    ? describe.skip
    : describe("DynamicSvgNFT Unit Test", () => {
          let deployer;
          let dynamicSvgNft: DynamicSvgNft;
          let mockV3Aggregator: MockV3Aggregator;
          beforeEach(async () => {
              const accounts = await ethers.getSigners();
              deployer = accounts[0];
              await deployments.fixture(["mocks", "dynamicsvg"]);
              dynamicSvgNft = await ethers.getContract("DynamicSvgNft");
              mockV3Aggregator = await ethers.getContract("MockV3Aggregator");
          });

          describe("Constructor Testing", () => {
              it("Should Inilitize constructor correctly", async () => {
                  const priceFeed = await dynamicSvgNft.getPriceFeed();

                  expect(priceFeed).to.equal(mockV3Aggregator.address);
                  const lowSvgUri = await dynamicSvgNft.getLowSVG();
                  const highSvgUri = await dynamicSvgNft.getHighSVG();
                  expect(lowSvgUri).to.equal(lowSVGImageuri);
                  expect(highSvgUri).to.equal(highSVGImageUri);
              });
          });

          describe("mintNFT Function Testing", () => {
              it("Should emit events and mint NFT", async () => {
                  const value = ethers.utils.parseEther("1");
                  await expect(dynamicSvgNft.mintNFT(value)).to.emit(
                      dynamicSvgNft,
                      "DynamicNFTCreated"
                  );
                  const tokenCounter = await dynamicSvgNft.getTokenCounter();
                  expect(tokenCounter.toNumber()).to.equal(1);
              });
              it("Should change the tokenURI if price changes", async () => {
                  const value = ethers.utils.parseEther("1"); // 1 dollar per ether;
                  const txRespo = await dynamicSvgNft.mintNFT(value);
                  await txRespo.wait(1);
                  const hightokenUri = await dynamicSvgNft.tokenURI(0);
                  expect(hightokenUri).to.equal(highTokenUri);

                  const valueAlt = ethers.utils.parseEther("10000"); // 10000 dollar per ether;
                  const txRespoAlt = await dynamicSvgNft.mintNFT(valueAlt);
                  await txRespoAlt.wait(1);
                  const lowtokenUri = await dynamicSvgNft.tokenURI(1);
                  expect(lowtokenUri).to.equal(lowTokenUri);
              });
          });
      });
